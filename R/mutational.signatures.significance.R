...
signaturesSignificance <-
  function(x,
    ...
    if (verbose) {
      message(
        "Estimating the contribution of each signature to the fit with a total of ",
        nboot,
    ...
    if (num_processes == 1) {
        ...
        curr_alpha <-
          lapply(
            ...
            FUN = function(counts) {
              ...
              curr_counts_distribution <-
                as.numeric(curr_patient / rowSums(curr_patient))
              curr_boot_sampling <-
                table(
                  sample(
                    x = colnames(curr_patient),
                    size = curr_num_counts,
                    replace = TRUE,
                    prob = curr_counts_distribution
                  )
                )
              curr_patient[1,] <- 0
              curr_patient[1, names(curr_boot_sampling)] <-
                as.numeric(curr_boot_sampling)
              return(
                .sigs_significance(
                  x = curr_patient,
                  beta = beta,
      ...
      clusterExport(parallel,
                    varlist = c(".sigs_significance"),
                    envir = environment())
      clusterExport(parallel,
                    varlist = c("verbose", "nboot"),
                    envir = environment())
      clusterExport(
        parallel,
        varlist = c("x", "beta", "cosine_thr", "sparsify"),
        envir = environment()
      )
      ...
      gc(verbose = FALSE)
      alpha <-
        parLapply(parallel, seq_len(nboot), function(boot_iteration) {
          if (verbose) {
            message(paste0(
              "Performing iteration ",
              boot_iteration,
      ...
      for (j in seq_len(ncol(pvalues))) {
        if (!is.na(pvalues[i, j])) {
          ...
          for (k in seq_len(length(alpha_distibution))) {
            curr_values <-
              c(curr_values, (alpha_distibution[[k]][i,] / sum(alpha_distibution[[k]][i,]))[j])
          }
          pvalues[i, j] <-
            wilcox.test(as.numeric(curr_values),
                        alternative = "greater",
    ...
    for (i in seq_len(nrow(x))) {
      ...
      goodness_fit <-
        c(goodness_fit, as.numeric(cosine(as.numeric(x[i,]), as.numeric((alpha[i,] %*%
                                                                           beta)
        ))))
    }
...
.sigs_significance <-
  function(x,
    ...
    sigs <-
      colnames(alpha[, which(alpha[1,] > 0), drop = FALSE])[sort.int(alpha[, which(alpha[1,] >
    ...
    for (i in seq_len(length(sigs))) {
      ...
      curr_goodness_fit <-
        as.numeric(cosine(as.numeric(x), as.numeric(curr_predicted_counts)))
