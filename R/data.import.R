...
getSBSCounts <- function(data, reference = NULL) {
  ...
  colnames(data) <-
    c("sample", "chrom", "start", "end", "ref", "alt")
  
  # consider only single nucleotide variants involving (A,C,G,T) bases
  data <-
    data[which(data[, "start"] == data[, "end"]), , drop = FALSE]
  data <-
    data[which(as.character(data[, "ref"]) %in% c("A", "C", "G", "T")), , drop =
           FALSE]
  data <-
    data[which(as.character(data[, "alt"]) %in% c("A", "C", "G", "T")), , drop =
           FALSE]
  data <-
    data[, c("sample", "chrom", "start", "ref", "alt"), drop = FALSE]
  colnames(data) <- c("sample", "chrom", "pos", "ref", "alt")
  data <- unique(data)
  data <-
    data[order(data[, "sample"], data[, "chrom"], data[, "pos"]), , drop =
           FALSE]
  ...
  data$cat <-
    ifelse(
      as.character(data$ref) %in% c("C", "T"),
      paste0(
        subseq(data$context, 1, 1),
        "[",
        data$ref,
        ">",
        data$alt,
        "]",
        subseq(data$context, 3, 3)
      ),
      paste0(
        subseq(data$rccontext, 1, 1),
        "[",
        data$cref,
        ">",
        data$calt,
        "]",
        subseq(data$rccontext, 3, 3)
      )
    )
  ...
  for (i in c("A", "C", "G", "T")) {
    for (j in seq_len(6)) {
      for (k in c("A", "C", "G", "T")) {
        cont <- cont + 1
        categories_context <-
          c(categories_context, paste0(k, ":", i))
        categories_cat <-
          c(categories_cat, paste0(k, "[", categories_alt[cont], "]", i))
      }
    }
  }
  mutation_categories <-
    data.table(context = categories_context,
               alt = categories_alt,
               cat = categories_cat)
  ...
  data <-
    merge(
      mutation_categories[, list(cat)],
      data.table(sample = data$sample, cat = data$cat)[, .N, by = list(sample, cat)],
      by = "cat",
      all = TRUE
    )
  ...
  colnames(trinucleotides_counts) <-
    sort(as.character(mutation_categories$cat))
  rows_contexts <- rownames(data)
  cols_contexts <-
    colnames(trinucleotides_counts)[which(colnames(trinucleotides_counts) %in%
                                            colnames(data))]
  trinucleotides_counts[rows_contexts, cols_contexts] <-
    data[rows_contexts, cols_contexts]
...
getMNVCounts <- function(data) {
  ...
  cond1 <-
    c(data$sample[seq_len((nrow(data) - 1))] == data$sample[2:nrow(data)], FALSE)
  cond2 <-
    c(data$chrom[seq_len((nrow(data) - 1))] == data$chrom[2:nrow(data)], FALSE)
  cond3 <-
    c(data$pos[seq_len((nrow(data) - 1))] == (data$pos[2:nrow(data)] - 1), FALSE)
  data <-
    cbind(data[which(cond1 &
                       cond2 & cond3),], data[(which(cond1 &
                                                       cond2 &
                                                       cond3) + 1), c("ref", "alt")])
  ...
  mutation_categories_complements <-
    c("GT>TG",
      "GT>CG",
      "GT>AG",
      "GT>TC",
  ...
  for (i in seq_len(length(mutation_categories_complements))) {
    data$cat[which(data$cat == mutation_categories_complements[i])] <-
      mutation_categories[i]
  }
  ...
  colnames(multi_nucleotides_counts) <-
    sort(mutation_categories$cat)
  rows_contexts <- rownames(data)
  cols_contexts <-
    colnames(multi_nucleotides_counts)[which(colnames(multi_nucleotides_counts) %in%
                                               colnames(data))]
  multi_nucleotides_counts[rows_contexts, cols_contexts] <-
    data[rows_contexts, cols_contexts]
