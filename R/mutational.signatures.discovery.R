    ...
    if (sparsify) {
      for (j in seq_len(nrow(alpha))) {
        ...
        if (nrow(curr_beta_values) > 1) {
          ...
          alpha[j,] <-
            as.numeric(res$glmnet.fit$beta[, ncol(res$glmnet.fit$beta)])
        }
        else {
          res_inputs <-
            cbind(t(curr_beta_values), matrix(rep(0, ncol(
              curr_beta_values
            )), ncol = 1))
          res <-
            cv.glmnet(
  ...
  function(x,
    ...
    if (any(colSums(x) == 0)) {
      ...
      for (inv_cols in invalid_cols) {
        x[sample(seq_len(length(x[, inv_cols])), size = 1), inv_cols] <-
          1e-05
      }
    }
    K <- sort(unique(K))
    ...
    if (num_processes == 1) {
      ...
      clusterExport(
        pbackend,
        varlist = c(".nmf_seed", ".nmf_nnls", ".nmf_lasso"),
        envir = environment()
      )
        ...
        rank0_measures <-
          matrix(
            c(
              ...
              NA,
              NA,
              rank0_sparseness_alpha,
              rank0_sparseness_beta,
              1,
              1,
              1
            ),
    ...
    if (length(K) > 0) {
      ...
      for (i in seq_len(length(K))) {
        ...
        # rescale alpha to the original magnitude
        if (normalize_counts) {
          alpha[[paste0(K[i], "_signatures")]] <-
            alpha[[paste0(K[i], "_signatures")]] * (rowSums(x_not_normalized) / 2500)
        }
        ...
        curr_sparseness_alpha <-
          sparseness(as.vector(alpha[[paste0(K[i], "_signatures")]]))
        curr_sparseness_beta <-
          sparseness(as.vector(beta[[paste0(K[i], "_signatures")]]))
        if (nmf_runs > 1) {
          ...
          curr_measures <-
            matrix(
              c(
                ...
                curr_rss,
                curr_evar,
                curr_silhouette_alpha,
                curr_silhouette_beta,
                curr_sparseness_alpha,
                curr_silhouette_beta,
                cophcor(results),
                dispersion(results),
                curr_silhouette_consensus
              ),
              nrow = 1
            )
          colnames(curr_measures) <-
            c(
              "Rank",
              "Residual_Sum_Squares",
              "Explained_Variance",
...
signaturesCV <-
  function(x,
    ...
    if (verbose) {
      message(
        "Estimating the optimal number of signatures with a total of ",
    ...
    cv_estimates <-
      array(NA, c(cross_validation_repetitions, length(beta)))
    rownames(cv_estimates) <-
      paste0("Repetition_", seq_len(cross_validation_repetitions))
    ...
    if (num_processes == 1) {
      ...
      for (cv_repetitions in seq_len(cross_validation_repetitions)) {
        ...
        cv_entries <-
          valid_entries[sample(
            seq_len(nrow(valid_entries)),
            size = round(nrow(valid_entries) * cross_validation_entries),
            replace = FALSE
          ),]
        ...
        for (num_signs in seq_len(length(beta))) {
          ...
          for (cv_iteration in seq_len(cross_validation_iterations)) {
            if (verbose) {
              message(
                paste0(
                  "Performing cross validation iteration ",
                  cv_iteration,
                  " out of ",
            ...
            if (cv_iteration == 1) {
              ...
              predicted_counts <-
                curr_results[["alpha"]] %*% curr_results[["beta"]]
              x_cv[cv_entries] <-
                predicted_counts[cv_entries]
            }
            ...
            if (any(colSums(x_cv) == 0)) {
              ...
              for (inv_cols in invalid_cols) {
                x_cv[sample(seq_len(length(x_cv[, inv_cols])), size = 1), inv_cols] <-
                  1e-05
              }
            }
            ...
            curr_results <-
              .nmf_fit(x = x_cv, beta = beta[[num_signs]])
            gc(verbose = FALSE)
            
          }
          
          # save an estimate of the current solution
          curr_predicted_counts <-
            curr_results[["alpha"]] %*% curr_results[["beta"]]
          rm(curr_results)
          curr_true_considered_counts <-
            as.vector(x[cv_entries])
          curr_predicted_considered_counts <-
            as.vector(curr_predicted_counts[cv_entries])
          error <-
            mean((
              curr_true_considered_counts - curr_predicted_considered_counts
            ) ^ 2)
          rm(curr_true_considered_counts)
          rm(curr_predicted_considered_counts)
          cv_estimates[cv_repetitions, paste0(k, "_signatures")] <-
            error
          rm(error)
          gc(verbose = FALSE)
      ...
      gc(verbose = FALSE)
      curr_results <-
        parLapply(parallel, seq_len(cross_validation_repetitions), function(cv_repetitions) {
          if (verbose) {
            message(
              paste0(
                "Performing repetition ",
          ...
          for (num_signs in seq_len(length(beta))) {
            if (cv_repetitions == 1) {
              cv_names <-
                c(cv_names, paste0(nrow(beta[[num_signs]]), "_signatures"))
            }
...
.nmf_seed <- function(model, target) {
  ...
  beta <-
    (beta / rowSums(beta)) # beta rows (i.e., signatures) must sum to 1
...
.nmf_nnls <- function(x, seed, background = NULL) {
  ...
  if (!is.null(background)) {
    # if specified, first row of beta is background
    beta[1,] <- background
  }
  n <-
    nrow(x) # n is the number of observations in x, i.e., the patients
  J <-
    ncol(x) # J is the number of trinucleotides, i.e., 96 categories
  ...
  for (i in seq_len(20)) {
    ...
    for (k in seq_len(J)) {
      if (!is.null(background)) {
        ...
        beta[2:K, k] <-
          nnls(alpha[, 2:K, drop = FALSE], as.vector(x[, k] - (alpha[, 1] * beta[1, k])))$x
      }
      else {
  ...
  if (!is.null(background)) {
    colnames(alpha) <-
      c("Background", paste0("S", seq_len((ncol(
        alpha
      ) - 1))))
  }
  else {
      ...
      if (!is.null(background)) {
        ...
        if (ncol(curr_alpha_values) > 1) {
          res <-
            cv.glmnet(
              x = curr_alpha_values,
              y = as.vector(x[, k] - (alpha[, 1] * beta[1, k])),
              type.measure = "mse",
              nfolds = 10,
              nlambda = 10,
              family = "gaussian",
          ...
          res_inputs <-
            cbind(curr_alpha_values, rep(0, nrow(curr_alpha_values)))
          res <-
            cv.glmnet(
              x = res_inputs,
              y = as.vector(x[, k] - (alpha[, 1] * beta[1, k])),
