...
groupsSBSPlot <-
  function(counts,
    ...
    for (i in seq_len(length(groups))) {
      ...
      plot_data <-
        data.frame(
          Context = paste0(substr(x_label, 1, 1), ".", substr(x_label, 7, 7)),
          alt = paste0(substr(x_label, 3, 3), ">", substr(x_label, 5, 5)),
          value = x_value
        )
      ...
      if (!xlabels) {
        plt <-
          plt + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
      }
...
groupsMNVPlot <-
  function(counts,
    ...
    for (i in seq_len(length(groups))) {
      ...
      plot_data <-
        data.frame(
          Context = substr(as.character(x_label), 4, 5),
          alt = paste0(substr(as.character(x_label), 1, 3), "NN"),
          value = x_value
        )
      plt <- ggplot(plot_data) +
        geom_boxplot(aes_string(x = "Context", y = "value", fill = "alt")) +
...
#'
patientsSBSPlot <-
  function(trinucleotides_counts,
           samples = rownames(trinucleotides_counts),
           freq = FALSE,
           xlabels = FALSE) {
    # make samples data
    trinucleotides_counts <-
      trinucleotides_counts[samples, , drop = FALSE]
    if (freq) {
      trinucleotides_counts <-
        trinucleotides_counts / rowSums(trinucleotides_counts)
    }
    
    # separate context and alteration
    x <-
      as.data.table(melt(
        as.matrix(trinucleotides_counts),
    ...
    for (i in seq_len(nrow(trinucleotides_counts))) {
      plt <- ggplot(x[x$patient == rownames(trinucleotides_counts)[i]]) +
        geom_bar(
          aes_string(x = "Context", y = "value", fill = "alt"),
          stat = "identity",
          position = "identity"
        ) +
        facet_wrap(~ alt, nrow = 1, scales = "free_x") +
        ...
        ggtitle(rownames(trinucleotides_counts)[i]) + theme(legend.position =
                                                              "none") + ylab("Number of mutations")
    ...
    # make the final plot
    grid.arrange(grobs = glist, ncol = ceiling(nrow(trinucleotides_counts) /
                                                 3))
...
#'
patientsMNVPlot <-
  function(multi_nucleotides_counts,
           samples = rownames(multi_nucleotides_counts),
           freq = FALSE,
           xlabels = FALSE) {
    # make samples data
    multi_nucleotides_counts <-
      multi_nucleotides_counts[samples, , drop = FALSE]
    if (freq) {
      multi_nucleotides_counts <-
        multi_nucleotides_counts / rowSums(multi_nucleotides_counts)
    }
    
    # separate context and alteration
    x <-
      as.data.table(melt(
        as.matrix(multi_nucleotides_counts),
        varnames = c("patient", "cat")
      ))
    ...
    for (i in seq_len(nrow(multi_nucleotides_counts))) {
      plt <-
        ggplot(x[x$patient == rownames(multi_nucleotides_counts)[i]]) +
        geom_bar(
          aes_string(x = "Context", y = "value", fill = "alt"),
          stat = "identity",
          position = "identity"
        ...
        ggtitle(rownames(multi_nucleotides_counts)[i]) + theme(legend.position =
                                                                 "none") + ylab("Number of mutations")
    ...
    # make the final plot
    grid.arrange(grobs = glist, ncol = ceiling(nrow(multi_nucleotides_counts) /
                                                 3))
...
signaturesSBSPlot <-
  function(beta,
    ...
    # separate context and alteration
    x <-
      as.data.table(melt(as.matrix(beta), varnames = c("signature", "cat")))
    x[, "Context" := paste0(substr(cat, 1, 1), ".", substr(cat, 7, 7))]
    x[, "alt" := paste0(substr(cat, 3, 3), ">", substr(cat, 5, 5))]
...
groupsCNPlot <-
  function(counts,
    ...
    for (i in seq_len(length(groups))) {
      ...
      for (j in as.character(x_label)) {
        Alt <-
          c(Alt, paste0(toupper(strsplit(j, ":")[[1]][[2]]), " ", strsplit(j, ":")[[1]][[1]]))
      }
...
patientsCNPlot <-
  function(cn_data_counts,
    ...
    # separate context and alteration
    x <-
      as.data.table(melt(as.matrix(cn_data_counts), varnames = c("patient", "cat")))
    Context <- NULL
    for (i in as.character(x$cat)) {
      Context <- c(Context, strsplit(i, ":")[[1]][[3]])
    }
    x[, "Context" := factor(Context,
                            levels = c(
                              "0-100kb",
                              "100kb-1Mb",
                              "1Mb-10Mb",
    ...
    for (i in as.character(x$cat)) {
      Alt <-
        c(Alt, paste0(toupper(strsplit(i, ":")[[1]][[2]]), " ", strsplit(i, ":")[[1]][[1]]))
    }
    Alt <- gsub("HOMDEL", "HD", Alt)
    Alt <- gsub("HET", "Het", Alt)
    x[, "alt" := factor(
    ...
    for (i in seq_len(nrow(cn_data_counts))) {
      plt <- ggplot(x[x$patient == rownames(cn_data_counts)[i]]) +
        ...
        ggtitle(rownames(cn_data_counts)[i]) + theme(legend.position =
                                                       "none") + ylab("Number of mutations")
...
patientsCXPlot <-
  function(cn_data_counts,
    ...
    alt <-
      factor(Alt_values,
             levels = c("Segment size", "Changepoint", "Breakpoints", "Oscillating"))
    x[, "Context" := Context]
    x[, "alt" := alt]
