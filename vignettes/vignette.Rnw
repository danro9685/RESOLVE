...
%\VignetteIndexEntry{SparseSignaturesPlus}
...
\section{Using the SparseSignaturesPlus R package}
...
imported_data = get.SBS.counts(data = ssm560_reduced, reference = BSgenome.Hsapiens.1000genomes.hs37d5)
head(imported_data)
@
...
patients.SBS.plot(trinucleotides_counts=imported_data,samples="PD10010a")
@
...
signatures.SBS.plot(beta=signatures, xlabels=FALSE)
@
